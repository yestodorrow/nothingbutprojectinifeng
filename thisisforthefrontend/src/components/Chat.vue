  ...
  <div class="chat" :class="{over: end}">    
    ...
    <div class="content" ref="content" @click="next" >
          <transition-group name="fade" tag="ul" v-on:enter="enter" ref="container">
            <li v-for="(item, index) in msg" :key="index" :item="item" :index="index"  is="message"></li>
          </transition-group>      
    ...
    <Tail v-if="!isIfeng()"/>   
    <div class="nav" v-if="!isIfeng()"><Navigator /></div>  
    <div class="steps"><IProgress :value="percent"/></div>
    <OpenApp v-if="!isNative()&&!isIfeng()"/>
  </div>
</template>
...
export default {
  ...
  watch: {    
    story(v,ov){            
      this.current = +storage.getItem(`${this.$store.state.book._id}-${this.$store.state.chapter.cid}`) || this.start-1;
    ...
    current(v, ov) {
      ...
      storage.setItem(`${this.$store.state.book._id}-${this.$store.state.chapter.cid}`, v);      
  ...
  methods: {    
    ...
    isIfeng(){
      return this.$store.state.channel == "102"
    },
    next() {      
        if(this.auto){
          this.auto = false;
